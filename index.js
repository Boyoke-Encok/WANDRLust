...
async function processAccount(account, index, total, proxy) {
  ...
  try {
    ...
    let currentCookies = [`referral_code=MOABR9YK`, ...nonceData.setCookie.map(ck => ck.split('; ')[0])];
...
async function run() {
  ...
  console.log(gradient.retro(centerText('=== Telegram Channel 🚀 : https://t.me/mbokatklalen ===', terminalWidth)));
